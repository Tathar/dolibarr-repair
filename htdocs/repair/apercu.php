...
/**
 *		\file		htdocs/repair/apercu.php
 *		\ingroup	repair
 *		\brief		Page de l'onglet apercu d'une repair
 */
...
llxHeader('',$langs->trans('Order'),'EN:Customers_Orders|FR:Repairs_Clients|ES:Pedidos de clientes');
...
if ($id > 0 || ! empty($ref))
{
	...
	if ($object->fetch($id,$ref) > 0)
	{
        ...
        dol_fiche_head($head, 'preview', $langs->trans("CustomerRepair"), 0, 'repair@repair');
		...
		$dir_output = $conf->repair->dir_output . "/";
		...
		if (file_exists($file))
		{
			...
			print '<td><a href="'.DOL_URL_ROOT . '/document.php?modulepart=repair&file='.urlencode($relativepath).'">'.$object->ref.'.pdf</a></td>';
			...
			{
				print "<tr $bc[$var]><td>Repair detaillee</td>";

				print '<td><a href="'.DOL_URL_ROOT . '/document.php?modulepart=repair&file='.urlencode($relativepathdetail).'">'.$object->ref.'-detail.pdf</a></td>';
...
{
	print '<img src="'.DOL_URL_ROOT . '/viewimage.php?modulepart=apercurepair&file='.urlencode($relativepathimage).'">';
}
...
elseif (file_exists($fileimagebis))
{
	...
	for ($i = 0; $i < 20; $i++)
	{
		...
		{
			print '<img src="'.DOL_URL_ROOT . '/viewimage.php?modulepart=apercurepair&file='.urlencode($preview).'"><p>';
		}
	}
}
