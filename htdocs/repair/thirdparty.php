	...
	if ($conf->repair->enabled && $user->rights->repair->read)
	{
		...
		if ($resql)
		{
			...
			if ($num > 0)
			{
    			...
    			print '<td colspan="4"><table width="100%" class="nobordernopadding"><tr><td>'.$langs->trans("LastRepairs",($num<=$MAXLIST?"":$MAXLIST)).'</td><td align="right"><a href="'.DOL_URL_ROOT.'/repair/liste.php?socid='.$object->id.'">'.$langs->trans("AllRepairs").' ('.$num.')</a></td>';
                print '<td width="20px" align="right"><a href="'.DOL_URL_ROOT.'/repair/stats/index.php?socid='.$object->id.'">'.img_picto($langs->trans("Statistics"),'stats').'</a></td>';
    			print '</tr></table></td>';
    			print '</tr>';
			}
