...
/**
 *	\file       htdocs/repair/document.php
 *	\ingroup    order
 *	\brief      Page de gestion des documents attachees a une repair
 */
...
if ($_POST["sendit"] && ! empty($conf->global->MAIN_UPLOAD_DOC))
{
	if ($object->fetch($id))
    {
	    ...
	    $upload_dir = $conf->repair->dir_output . "/" . dol_sanitizeFileName($object->ref);
...
if ($id > 0 || ! empty($ref))
{
	if ($object->fetch($id, $ref))
    {
		...
		dol_fiche_head($head, 'documents', $langs->trans('CustomerRepair'), 0, 'repair@repair');
		...
		$formfile->form_attach_new_file(DOL_URL_ROOT.'/repair/document.php?id='.$object->id,'',0,0,$user->rights->repair->creer,50,$object);


		// List of document
		$param='&id='.$object->id;
		$formfile->list_of_documents($filearray,$object,'repair',$param);
	}
	else
	{
		dol_print_error($db);
...
print $conf->global->MAIN_DELAY_REPAIR_TO_PROCESS;
llxFooter();
