...
/**
 *	    \file       htdocs/repair/stats/index.php
 *      \ingroup    repair
...
if ($mode == 'supplier' && ! $user->rights->fournisseur->repair->read) accessforbidden();
...
if ($mode == 'customer')
{
    $title=$langs->trans("RepairsStatistics");
    $dir=$conf->repair->dir_temp;
}
if ($mode == 'supplier')
{
    $title=$langs->trans("RepairsStatisticsSuppliers");
...
$stats = new RepairStats($db, $socid, $mode, $userid);
...
if (! $mesg)
{
    ...
    $px1->SetYLabel($langs->trans("NbOfRepair"));
    $px1->SetShading(3);
    $px1->SetHorizTickIncrement(1);
    $px1->SetPrecisionY(0);
    $px1->mode='depth';
    $px1->SetTitle($langs->trans("NumberOfRepairsByMonth"));
...
if (! $mesg)
{
    ...
    $px2->SetYLabel($langs->trans("AmountOfRepairs"));
    $px2->SetShading(3);
    $px2->SetHorizTickIncrement(1);
    $px2->SetPrecisionY(0);
    $px2->mode='depth';
    $px2->SetTitle($langs->trans("AmountOfRepairsByMonthHT"));
...
$h=0;
$head = array();
$head[$h][0] = DOL_URL_ROOT . '/repair/stats/index.php?mode='.$mode;
...
print '<td align="center">'.$langs->trans("NbOfRepairs").'</td>';
