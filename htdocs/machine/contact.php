...
/**
 *     \file       htdocs/repair/contact.php
 *     \ingroup    repair
 *     \brief      Onglet de gestion des contacts de repair
 */
...
else if ($action == 'setaddress' && $user->rights->repair->creer)
{
	$object->fetch($id);
...
llxHeader('',$langs->trans('Repair'),'EN:Customers_Repairs|FR:Repairs_Clients|ES:Pedidos de clientes');
...
if ($id > 0 || ! empty($ref))
{
	...
	if ($object->fetch($id, $ref) > 0)
	{
		...
		dol_fiche_head($head, 'contact', $langs->trans("CustomerRepair"), 0, 'repair@repair');
		...
		*/
		...
		if ($conf->global->SOCIETE_ADDRESSES_MANAGEMENT)
		{
			...
			{
				$formother->form_address($_SERVER['PHP_SELF'].'?id='.$object->id,$object->fk_delivery_address,GETPOST('socid','int'),'fk_address','repair',$object->id);
			}
			else
			{
				$formother->form_address($_SERVER['PHP_SELF'].'?id='.$object->id,$object->fk_delivery_address,GETPOST('socid','int'),'none','repair',$object->id);
			}
			print '</td></tr>';
		}
